...
### 할당
변수의 이름에 `할당 연산자(=)` 를 사용하여 **값을 할당**하는 단계이다.  
```js
var name; // 선언
name = "cookie"; // 할당
```
...
코드 평가 과정에서는 **함수나 변수의 선언 부분을 맨 위로 끌어올려 메모리 공간을 미리 할당하는** 호이스팅 과정이 일어나는데 `var` 와 `let` 으로 선언한 변수에는 차이가 존재한다.  
...
`var` 로 선언한 변수는 전역 객체에, `let` 으로 선언한 변수는 전역 객체가 아닌 전역 블럭 개념의 영역에 저장되고 있다.

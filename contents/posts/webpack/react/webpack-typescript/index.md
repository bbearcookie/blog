...
왜 `Webpack` 으로 구성했는지 묻는다면.. 물론 `Vite`는 Webpack 기반이 아니라 개발 환경 및 프로덕션 환경에 따라서 esbuild와 Rollup을 번들러로 사용하지만, 번들러와 트랜스파일러에 대해 전반적인 이해를 하는데 있어서는 Webpack 기반으로 구성해보는 것이 나쁘지 않겠다는 생각이었다.

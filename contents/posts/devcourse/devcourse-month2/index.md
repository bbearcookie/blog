...
<b style="color: red">**복제의 원형이 바로 함수 객체의 `prototype` 프로퍼티가 가리키는 객체**</b>라는 내용도 이해하기 어려웠다. 내 생각에는 프로토타입은 복제라기보다는 <b style="color: blue">**객체끼리의 연결 관계를 포인터로 표현하는 것**</b>에 가까워보였기 때문에 너무 난해하게 느껴졌다.
...
그래도 발표를 마치니 나름 뿌듯하기도 하고 객체지향이라고 하는 난해한 주제에 대해서 한 발자국 더 나아간 이해를 하게 된 느낌이다. 특히 클래스 기반의 프로그래밍과 프로토타입 기반의 프로그래밍의 차이가 무엇인지, 왜 달라야 하는지 이해하지 못한 채 그냥 JS에서는 객체끼리 연결해서 상속 관계를 표현하는구나.. 정도만 이해하고 프로토타입과 관련한 프로퍼티를 난해하게 외우고 있었구나 싶은 생각이 들었다.
...
물론 최근 리액트는 함수형 패러다임으로 컴포넌트를 작성하는 추세이긴 하지만 그래도 나중에 여유가 된다면 알아보면 좋은 주제가 될 것 같다!

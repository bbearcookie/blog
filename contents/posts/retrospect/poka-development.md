...
졸업 작품으로 했었던 프로젝트가 아이디어는 좋았다고 생각하기 때문에 그 소재를 가지고 아쉬웠던 부분은 개선하고, 새로운 라이브러리를 적용하고, 코드를 작성할 때 어떤 방식으로 작성하면 좋은지 충분히 고민하고 정리하면서 프론트엔드부터 백엔드까지 결과물을 새로 만들어 보기로 했다.
...
## 아쉬웠던 점
혼자 진행하는 프로젝트이다보니 코드를 어떻게 작성해야 깔끔하고 관리하기 좋은지를 여러 번 고민했지만, 좋은 결론을 내리는게 쉽지 않았다.
...
그렇다보니 프로젝트에 필요한 새로운 기능을 구현하기 보다 기존에 작성했던 코드를 수정하는데 더 시간을 할애하게 되었고, 자연스럽게 프로젝트가 완성되기까지 오래 걸리다보니 흥미가 점점 떨어지게 되었다.  

또 이번에 새롭게 사용했던 기술이 `TypeScript`, `React-Query`, `Express-Validator` 였는데 우선 적용하고 사용해보는 방식으로 학습을 했었다.  

그렇다보니 정확하게 이해하지 못한 채로 사용하다가 나중에 수정해야 하는 부분들이 생기는 시행 착오를 많이 겪었다.
...
### 테스트 코드
기존에 작성한 코드를 리팩토링 할 때마다 실수로 인해서 버그가 발생하지는 않았는지 일일히 기능을 테스트해보는 과정이 복잡했고 시간도 많이 소모됐었다. 최근에 `Jest` 를 알게 되었는데, `Jest` 로 테스트 코드를 작성하면 리팩토링 하는 과정이 훨씬 수월해보였다. 그래서 `Jest` 를 학습하고 다음 프로젝트에는 적용해봐야겠다.
